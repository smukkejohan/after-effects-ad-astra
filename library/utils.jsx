	...
	for (var i = 1; i <= proj.numItems; i++) {
		...
		if (item === comp || item.name === comp) {
			if (item instanceof CompItem) {
				...
				item.clone = function (compName) {
					return utils.cloneComp(item, compName);
				}
				item.getLayer = function (layerName) {
					return utils.getLayer(item, layerName);
				}
				item.layerExists = function (layerName) {
					return utils.layerExists(item, layerName);
				}
				item.searchLayer = function (str) {
					return utils.searchForLayer(item, str);
				}
				item.searchLayers = function (str) {
					return utils.searchForLayers(item, str);
				}
				item.clear = function () {
					return utils.clearComp(item);
				}
				item.mute = function () {
					return utils.muteCompAndAllNestedLayers(item);
				}
				item.disableAll = function () {
					return utils.disableAllLayers(item);
				}
				item.enableAll = function () {
					return utils.enableAllLayers(item);
				}
				item.getCamera = function () {
					return utils.getCamera(item);
				}
				item.setupCamera = function () {
					return utils.setupCamera(item);
				}
				item.addToComp = function (comp, duration) {
					return utils.addToComp(item, comp, duration);
				}
				item.getRandomLayer = function () {
					return utils.getRandomLayer(item);
				}
				item.getAllCompLayers = function () {
					return utils.getAllCompLayers(item);
				}
				item.getAllLayers = function () {
					return utils.getAllLayers(item);
				}
				item.moveAllKeys = function (inTime, outTime, offset) {
					return utils.moveAllKeysOnAllLayers(item, inTime, outTime, offset);
				}
				item.changeResolution = function (width, height, pixelAspect) {
					return utils.changeCompResolution(item, width, height, pixelAspect);
				}
				return item;
			}
		}
...
utils.getItem = function (itemName) {
	...
	for (var i = 1; i <= proj.numItems; i++) {
		...
		if (item === itemName || item.name === itemName) {
			...
			item.addToComp = function (comp) {
				return utils.addToComp(this, comp);
			}
			return item;
		}
	}
...
utils.enhanceLayer = function (layer) {
	...
	layer.setText = function (str) {
		return utils.setLayerText(this, str);
	};
	layer.resetParagraphStyle = function () {
		return utils.resetParagraphStyle(this);
	};
	layer.getText = function () {
		return utils.getLayerText(this);
	};
	layer.getFontSize = function () {
		return utils.getFontSize(this);
	};
	layer.setFont = function (font) {
		return utils.setLayerFont(this, font);
	};
	layer.setTextColor = function (color) {
		return utils.setLayerTextColor(this, color);
	};
	layer.enable = function () {
		return utils.enableLayer(this);
	};
	layer.disable = function () {
		return utils.disableLayer(this);
	};
	layer.getScale = function (time) {
		return utils.getLayerScale(this, time);
	};
	layer.setScale = function (time) {
		return utils.setLayerScale(this, time);
	};
	layer.getWidth = function (time) {
		return utils.getLayerWidth(this, time);
	}; // will be deprecated
	layer.getHeight = function (time) {
		return utils.getLayerHeight(this, time);
	}; // will be deprecated
	layer.getSize = function (time) {
		return utils.getLayerSize(this, time);
	};
	layer.getTextSize = function (str, time) {
		return utils.getTextLayerSize(this, str, time);
	};
	layer.clone = function (str) {
		return utils.cloneLayer(this, str);
	};
	layer.cloneComp = function (str) {
		return utils.cloneLayersComp(this, str);
	};
	//layer.deepClone       = function(str)                            			{ return utils.deepCloneLayer(this, str); };
	layer.scaleToHD = function (time) {
		return utils.scaleLayerToHD(this, time);
	};
	layer.setTimeRemap = function (time) {
		return utils.setTimeRemap(this, time);
	};
	layer.holdFrameAtTime = function (time) {
		return utils.holdFrameAtTime(this, time);
	};
	layer.setStartTime = function (time) {
		return utils.setStartTime(this, time);
	};
	layer.setEndTime = function (time) {
		return utils.setEndTime(this, time);
	};
	layer.addToComp = function (comp, duration) {
		return utils.copyLayerToComp(this, comp, duration);
	};
	layer.getComp = function () {
		return utils.getCompFromLayer(this);
	};
	layer.isComp = function () {
		return (this.source instanceof CompItem)
	};
	layer.getMarkerIndex = function (comment) {
		return utils.getLayerMarkerIndexByComment(this, comment)
	};
	layer.getMarkerTime = function (comment) {
		return utils.getLayerMarkerTimeByComment(this, comment)
	};
	layer.getMask = function (name) {
		return utils.getMaskByName(this, name);
	};
	layer.addMask = function (arr) {
		return utils.addMaskToLayer(this, arr);
	};
	layer.masksToShapes = function () {
		return utils.rd_MasksToShapes_doIt(this);
	};
	layer.setPos = function (arr, time) {
		return utils.setLayerPosition(this, arr, time);
	};
	layer.getPos = function (time) {
		return utils.getLayerPosition(this, time);
	};
	layer.setAnchor = function (arr, time) {
		return utils.setLayerAnchorPoint(this, arr, time);
	};
	layer.getAnchor = function (time) {
		return utils.getLayerAnchorPoint(this, time);
	};
	layer.snapGrid = function (gx, gy, time) {
		return utils.snapLayerToGrid(this, gx, gy, time);
	};
	layer.getOffset = function (time) {
		return utils.getLayerOffset(this, time);
	};
	layer.getKey = function (propStr, index) {
		return utils.getKeyframeAtIndex(this, propStr, index);
	};
	layer.getKeys = function (propStr) {
		return utils.getKeyframes(this, propStr);
	};
	layer.setKey = function (propStr, value, time) {
		return utils.setKeyframeAtTime(this, propStr, value, time);
	};
	layer.deleteKey = function (propStr, time) {
		return utils.deleteKeyframeAtTime(this, propStr, time);
	};
	layer.updateKey = function (propStr, value, index) {
		return utils.updateKeyframeAtIndex(this, propStr, value, index);
	};
	layer.offsetAllKeys = function (prop, offset, preExpression) {
		return utils.offsetAllKeyframesForProp(this, prop, offset, preExpression);
	};
	layer.offsetKey = function (prop, index, offset, preExpression) {
		return utils.offsetKeyframeForProp(layer, prop, index, offset, preExpression);
	};
	layer.easeKey = function (prop, value1, value2, index) {
		return utils.setEaseAtKeyIndex(this, prop, value1, value2, index);
	};
	layer.selectKeys = function (inTime, outTime) {
		return utils.selectPropKeys(this, inTime, outTime);
	};
	layer.moveAllKeys = function (inTime, outTime, offset) {
		return utils.movePropKeys(this, inTime, outTime, offset);
	};
	layer.moveKeys = function (propStr, inTime, outTime, offset) {
		return utils.movePropKeys(this, propStr, inTime, outTime, offset);
	};
	layer.mute = function () {
		return utils.mute(this);
	};
	layer.unMute = function () {
		return utils.unMute(this);
	};
	layer.copyPasteKeys = function (keyStartTime, keyEndTime, toLayer, offset) {
		return utils.copyPasteKeys(this, keyStartTime, keyEndTime, toLayer, offset);
	};
	layer.updateMarkersFromSource = function () {
		return utils.updateMarkersFromSource(this);
	};
	...
	layer.pos = {
		...
		setX: function (x, time) {
			this.set(new Vec3(x, this.getY(), this.getZ()), time);
		},
		setY: function (y, time) {
			this.set(new Vec3(this.getX(), y, this.getZ()), time);
		},
		setZ: function (z, time) {
			this.set(new Vec3(this.getX(), this.getY(), z), time);
		},

		getX: function (time) {
			return this.get(time).x;
		},
		getY: function (time) {
			return this.get(time).y;
		},
		getZ: function (time) {
			return this.get(time).z;
		},

		getLowerLeft: function (time) {
			return utils.getLayerCorners(layer, time)[0];
		},
		getUpperRight: function (time) {
			return utils.getLayerCorners(layer, time)[1];
		},
	};
...
utils.findClosestPoint = function (p, vertices) {
	...
	var sortArray = distArray.sort(function (a, b) {
...
utils.enhanceMask = function (mask) {
	//mask.anchor = [0,0]
	mask.getAnchor = function () {
		return utils.getMaskAnchor(this);
	};
	mask.setAnchor = function (arr) {
		return utils.setMaskAnchor(this, arr);
	};
	mask.getPath = function () {
		return utils.getMaskPath(this);
	};
	mask.getVertices = function () {
		return utils.getMaskVertices(this);
	};
	mask.setVertices = function (arr) {
		return utils.setMaskVertices(this, arr);
	};
	mask.getWidth = function () {
		return utils.getMaskWidth(this);
	};
	mask.setWidth = function (w) {
		return utils.setMaskWidth(this, w);
	};
	mask.getHeight = function () {
		return utils.getMaskHeight(this);
	};
	mask.getMinX = function () {
		return utils.getMaskMinX(this);
	};
	mask.getMaxX = function () {
		return utils.getMaskMaxX(this);
	};
	mask.getMinY = function () {
		return utils.getMaskMinY(this);
	};
	mask.getMaxY = function () {
		return utils.getMaskMaxY(this);
	};
	mask.setPos = function (arr) {
		return utils.setMaskPosition(this, arr);
	};
	mask.getPos = function () {
		return utils.getMaskPosition(this);
	};
	mask.offset = function (arr) {
		return utils.offsetMaskPosition(this, arr);
	};
	mask.getAvrPoint = function () {
		return utils.getMaskPathAveragePoint(this);
	};
	mask.setDirection = function (str) {
		return utils.setMaskVerticesDirection(this, str);
	};
	mask.debugPairs = function () {
		return utils.roundMaskVerticesPairs(this);
	};
	return mask;
};
...
utils.getMaskWidth = function (mask) {
	...
	var sortedX = verts.sort(function (a, b) {
		return (a[0] < b[0] ? -1 : (a[0] > b[0] ? 1 : 0));
	});
...
utils.getMaskMinY = function (mask) {
	var verts = mask.getVertices();
	var sortedY = verts.sort(function (a, b) {
